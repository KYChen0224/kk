...
sars_cases <- read_csv("input_data/sars.csv")
countries = read_csv("input_data/countries_codes_and_coordinates.csv")
ebola_cases = read_csv("input_data/ebola.csv")
h1n1_cases = read_csv("input_data/h1n1.csv")
worldcountry = geojson_read("input_data/50m.geojson", what = "sp")
country_geoms = read_csv("input_data/country_geoms.csv")
cv_states = read_csv("input_data/coronavirus_states.csv")
...
add_variable <- function(file,filt,obj = 1) {
  file$weeks_since_case100 = 0
  file$weeks_since_death10 = 0
  if (obj == 1) {
    for (i in 1:length(filt)) {
      a_name = as.character(filt)[i]
      a_db = subset(file, country==a_name)
      a_db$weeks_since_case100[a_db$cases>=100] = 0:(sum(a_db$cases>=100)-1)
      a_db$weeks_since_death10[a_db$deaths>=10] = 0:(sum(a_db$deaths>=10)-1)
      file$weeks_since_case100[file$country==a_name] = a_db$weeks_since_case100
      file$weeks_since_death10[file$country==a_name] = a_db$weeks_since_death10
    }
  }  else {
    file$continent = file$continent_level
    for (i in 1:length(filt)) {
      a_name = as.character(filt)[i]
      a_db = subset(file, continent==a_name)
      a_db$weeks_since_case100[a_db$cases>=100] = 0:(sum(a_db$cases>=100)-1)
      a_db$weeks_since_death10[a_db$deaths>=10] = 0:(sum(a_db$deaths>=10)-1)
      file$weeks_since_case100[file$continent==a_name] = a_db$weeks_since_case100
      file$weeks_since_death10[file$continent==a_name] = a_db$weeks_since_death10
    }    
  }
  file
}
...
check_date <- function(file) {
  if (any(grepl("/", file$date))) { 
    file$date = format(as.Date(file$date, format="%d/%m/%Y"),"%Y-%m-%d") 
  } else { file$date = as.Date(file$date, format="%Y-%m-%d") }
  file
}

# check consistency of country names across datasets
if (sum(!(unique(cv_cases$country) %in% unique(countries$country)))>0){ print("Error: inconsistent country names")}

# extract dates from cv data
cv_cases = check_date(cv_cases)
...
cv_cases = cv_cases %>%
  arrange(date) %>%
  mutate(cases_per_million = as.numeric(format(round(cases/(population/1000000),1),nsmall=1)),
         new_cases_per_million = as.numeric(format(round(new_cases/(population/1000000),1),nsmall=1)),
         million_pop = as.numeric(population>1e6),
         deaths_per_million = as.numeric(format(round(deaths/(population/1000000),1),nsmall=1)),
         new_deaths_per_million = as.numeric(format(round(new_deaths/(population/1000000),1),nsmall=1)))

# add variable for weeks since 100th case and 10th death
uniq_country = unique(cv_cases$country)
cv_cases = add_variable(cv_cases,uniq_country)

# creat variable for today's data
#cv_today = subset(cv_cases, date==current_date) 
cv_today = cv_cases %>% filter(date==current_date)
...
cv_states = check_date(cv_states)
cv_states_today = cv_states %>% filter(date==max(date))

# create subset for countries with at least 1000 cases
cv_today_reduced = cv_today %>% filter(cases>=1000)
...
# add variable for weeks since 100th case and 10th death
uniq_continent = unique(cv_cases_continent$continent)
cv_cases_continent = add_variable(cv_cases_continent,uniq_continent,obj = 2)

# add continent populations
cv_cases_continent$pop = NA
cv_cases_continent<-cv_cases_continent%>%
  mutate(
    pop = ifelse(continent=="Africa", 1.2e9, pop),
    pop = ifelse(continent=="Asia", 4.5e9, pop),
    pop = ifelse(continent=="Europe", 7.4e8, pop),
    pop = ifelse(continent=="North America", 5.8e8, pop),
    pop = ifelse(continent=="Oceania", 3.8e7, pop),
    pop = ifelse(continent=="South America", 4.2e8, pop)
  )
...
cv_cases_continent = cv_cases_continent %>%
  mutate(cases_per_million = as.numeric(format(round(cases/(pop/1000000),1),nsmall=1)),
         new_cases_per_million = as.numeric(format(round(new_cases/(pop/1000000),1),nsmall=1)),
         deaths_per_million = as.numeric(format(round(deaths/(pop/1000000),1),nsmall=1)),
         new_deaths_per_million = as.numeric(format(round(new_deaths/(pop/1000000),1),nsmall=1)))
...
cv_cases_global = cv_cases_global %>%
  mutate(pop = 7.6e9,
         cases_per_million = as.numeric(format(round(cases/(pop/1000000),1),nsmall=1)),
         new_cases_per_million = as.numeric(format(round(new_cases/(pop/1000000),1),nsmall=1)),
         deaths_per_million = as.numeric(format(round(deaths/(pop/1000000),1),nsmall=1)),
         new_deaths_per_million = as.numeric(format(round(new_deaths/(pop/1000000),1),nsmall=1)))
...
ui <- bootstrapPage(
  ...
  navbarPage(theme = shinytheme("flatly"), collapsible = TRUE,
             ...
             tabPanel("Region plots",
                      ...
                      sidebarLayout(
                        ...
                        mainPanel(
                          ...
                          verbatimTextOutput("region_table")                          
                        )
                      )
             ),
             ...
             tabPanel("Outbreak comparisons",
                      ...
                      verbatimTextOutput("comparison_table")
             ),
             ...
             tabPanel("Data",
                      sidebarLayout(
                        ...
                        mainPanel(verbatimTextOutput("rawtable"))
                      )
             ),
...
server = function(input, output, session) {
  ...
  output$comparison_table <- renderPrint({
    epi_comp_sub = epi_comp
    names(epi_comp_sub) = c("outbreak", "cases", "deaths", "countries", "cfr")
    print(epi_comp_sub, row.names = FALSE)
  })
  ...
  observeEvent(input$level_select, {
    if (input$level_select=="Global") {
      ...
      updatePickerInput(session = session, inputId = "outcome_select", 
                        choices = c("Deaths per million", "Cases per million", "Cases (total)", "Deaths (total)"), selected = "Cases (total)")
      updatePickerInput(session = session, inputId = "start_date", 
                        choices = c("Date", "Week of 100th confirmed case", "Week of 10th death"), selected = "Date")
    }
    ...
    if (input$level_select=="US state") {
      ...
      updatePickerInput(session = session, inputId = "outcome_select", 
                        choices = "Cases (total)", selected = "Cases (total)")
      updatePickerInput(session = session, inputId = "start_date", 
...
# create dataframe with selected countries
  ...
  output$region_table <- renderPrint({
    region_table = country_reactive_db()
    orig <- options(width = 1000)
    print(tail(region_table, 20))
    options(orig)
  })
  ...
  output$rawtable <- renderPrint({
    cv_cases_sub = cv_cases %>% select(c(country, date, cases, new_cases, deaths, new_deaths,
      ...
      filter((date>input$range_date[1])&(date<input$range_date[2])) %>%
